...
const PRODUCT_CARDS = [
  {
    key: 'essentialService',
    price: 'US$25.00',
    status: 'active',
    sku: 'PRD-001'
  },
  {
    key: 'professionalPackage',
    price: 'US$120.00',
    status: 'active',
    sku: 'PRD-002'
  },
  {
    key: 'initialImplementation',
    price: 'US$200.00',
    status: 'inactive',
    sku: 'PRD-003'
  },
...
const ProductCard = ({ product, copy }) => (
  <div className="h-full w-full rounded-2xl bg-white border border-gray-200 p-6 flex flex-col justify-between">
    <div>
      <div className="text-xs uppercase tracking-wide text-gray-400">{copy.product}</div>
      <div className="text-xl font-semibold text-gray-900 mt-1">{copy.cardNames[product.key]}</div>
      <div className="text-sm text-gray-500 mt-2">SKU {product.sku}</div>
    </div>
    <div className="flex items-center justify-between mt-6">
      <div>
        <div className="text-xs text-gray-500">{copy.price}</div>
...
export default function IssuingShowcase() {
  ...
  const copyByLanguage = {
    es: {
      ...
      cardNames: {
        essentialService: 'Servicio esencial',
        professionalPackage: 'Paquete profesional',
        initialImplementation: 'Implementacion inicial',
        monthlySupport: 'Soporte mensual',
      },
    },
    en: {
      product: 'Product',
      ...
      cardNames: {
        ...
        initialImplementation: 'Initial implementation',
              ...
              >
                <div className="relative w-full h-full">
                  {PRODUCT_CARDS.map((product, index) => (
                    ...
                    >
                      <ProductCard product={product} copy={copy} />
                    </motion.div>
                  ))}
                </div>
              </motion.div>
                ...
                {PRODUCT_CARDS.map((product, index) => (
                  ...
                  >
                    <div className="h-full w-full rounded-xl bg-white border border-gray-200 p-4 flex flex-col justify-between">
                      <div>
                        <div className="text-[10px] uppercase tracking-wide text-gray-400">{copy.product}</div>
                        <div className="text-base font-semibold text-gray-900 mt-1">{copy.cardNames[product.key]}</div>
                        <div className="text-xs text-gray-500 mt-2">SKU {product.sku}</div>
                      </div>
                      <div className="flex items-center justify-between mt-4">
                        <div>
                          <div className="text-[10px] text-gray-500">{copy.price}</div>
